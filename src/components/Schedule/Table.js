...
const TableBody = (props) => {
    ...
    for (var i = 2; i <= 7; ++ i) {
        ...
        for (j = 0; j <= 12; ++ j) {
                ...
                if (inDaySubject[k].start === j) {
                    ...
                    resultDay.push(
                        <td 
                            ...
                            <div style={{fontWeight: 'bold'}}>{inDaySubject[k].courseName} ({inDaySubject[k].classID})</div> <br></br>
            ...
            if (have === false) {
                if (j === 0) resultDay.push(<td>Thứ {i}</td>)
                else resultDay.push(<td></td>)
            }
        }
        result.push(<tr>{resultDay}</tr>)
    }
